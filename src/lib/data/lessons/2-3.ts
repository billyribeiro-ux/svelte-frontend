...
<style>
  ...
  // ============================================================
  // SOLUTION — Conditional Rendering: {#if}
  // ============================================================
  // Best-practice version with TypeScript types and clean patterns.

  // 1. Simple toggle
  let showMessage: boolean = $state(false);

  // 2. Multi-state status flow with typed union
  type Status = 'idle' | 'loading' | 'success' | 'error';
  let status: Status = $state('idle');
  const statuses: Status[] = ['idle', 'loading', 'success', 'error'];

  function nextStatus(): void {
    const i = statuses.indexOf(status);
    status = statuses[(i + 1) % statuses.length];
  }
  ...
  let data: DataPayload | null = $state(null);
  ...
  // 5. Nested conditions: auth + role
  let loggedIn: boolean = $state(false);
  let isAdmin: boolean = $state(false);

  // 6. Live form validation with derived state
