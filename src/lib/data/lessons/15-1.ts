...
<style>
	...
	solution: [
		{
			...
			content: `<script lang="ts">
  // ============================================================
  // SOLUTION — {@attach} & use: Directive Awareness
  // ============================================================
  // Best-practice: fully typed factory attachments, proper cleanup,
  // composable patterns.

  // 1. Auto-focus (simplest — no params, no factory)
  function autofocus(el: HTMLElement): void {
    el.focus();
  }

  // 2. Tooltip factory — parameterized, returns cleanup
  function tooltip(text: string) {
    return (el: HTMLElement) => {
      const tip = document.createElement('div');
      tip.textContent = text;
      ...
      const hide = (): void => { tip.style.opacity = '0'; };
  ...
  // 6. Long press factory
  function longPress(ms: number, callback: () => void) {
    return (el: HTMLElement) => {
      let timer: number | null = null;
      const start = (): void => { timer = window.setTimeout(callback, ms); };
...
<section class="pitfalls">
  ...
  <ul class="pitfall-list">
    <li>
      <strong>Returning cleanup is optional but recommended</strong>
      Any listeners, observers, or timers should be torn down in the returned function.
    </li>
    <li>
      <strong>Factory vs inline function changes reactivity</strong>
      A factory called with current params captures them; reading $state inside the attachment body makes it re-run on change.
    </li>
    <li>
      <strong>Multiple {@attach} directives compose freely</strong>
      Stack as many as you need on one element.
    </li>
    <li>
      <strong>Prefer {@attach} over use: in new code</strong>
      Attachments integrate with runes and are the documented path forward.
    </li>
  </ul>
</section>
