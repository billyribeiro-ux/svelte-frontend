...
This lesson covers writing load functions, returning data, and consuming it in page components with the data prop — plus the full dependency-tracking story: depends() to opt IN to targeted invalidation, and untrack() to opt OUT when reading params or the URL shouldn't cause a rerun.`,
	objectives: [
		...
		'Exclude values from dependency tracking with untrack() to prevent unwanted reruns',
...
<script lang="ts">
  ...
  {:else if selectedExample === 'depends'}
    <section>
      <h2>depends(), untrack() & Targeted Invalidation</h2>
      <p>
...
// URL, so invalidate('/api/todos') also works if that URL is fetched.\`}</pre>
      ...
      <h3>untrack() — opting OUT of dependency tracking</h3>
      <p>
        Reading <code>params</code>, <code>url.pathname</code> or
        <code>url.searchParams.get(...)</code> inside load registers a dependency — the
        load reruns whenever that value changes. Occasionally that's not what you want.
        Wrap the read in <code>untrack()</code> to read the value <em>without</em>
        depending on it:
      </p>
      <pre>{\`// src/routes/+page.ts
import type { PageLoad } from './$types';

export const load: PageLoad = async ({ untrack, url, fetch }) => {
  // Read url.pathname WITHOUT making this load rerun
  ...
  return { content: await res.json() };
      ...
      <div class="demo-box">
        <p><strong>Tracking rules worth memorizing:</strong></p>
        <ul>
          <li>Search params are tracked <em>individually</em> — <code>url.searchParams.get('x')</code> reruns on <code>?x=</code> changes only, not <code>?y=</code> changes.</li>
          <li>Tracking stops once load returns — reading <code>params.x</code> inside a streamed promise does <strong>not</strong> register a dependency (dev mode warns you).</li>
          <li><code>await parent()</code> makes this load rerun whenever the parent does.</li>
        </ul>
      </div>
    </section>
...
export const load: PageLoad = async ({ params, fetch }) => {
  ...
  if (!res.ok) {
    // error() throws internally (no 'throw' keyword in SvelteKit 2)
    // and renders the nearest +error.svelte
  ...
  <section>
    ...
    <table>
      ...
      <tbody>
        ...
        <tr><td><code>untrack</code></td><td>Read a value without registering it as a dependency</td></tr>
