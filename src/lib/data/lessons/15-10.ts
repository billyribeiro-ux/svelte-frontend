...
This lesson provides a side-by-side comparison of the most important changes: onMount becomes $effect, writable/readable stores become $state/$derived, slots become snippets, and on:event directives become native event properties. Each comparison shows the Svelte 4 way alongside the Svelte 5 equivalent.`,
	objectives: [
  ...
  let activeTab: 'lifecycle' | 'state' | 'slots' | 'events' = $state('lifecycle');
...
<nav class="tabs">
  {#each ['lifecycle', 'state', 'slots', 'events'] as tab}
    <button
