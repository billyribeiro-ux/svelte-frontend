...
`,
  ...
  // ============================================================
  // SOLUTION — Context: Sharing Without Props
  // ============================================================
  // Best-practice: typed createContext, reactive objects in context,
  // clean provider/consumer separation.
...
<main>
  ...
  <section>
    <h2>Props vs context vs module state</h2>
    <ul>
      <li><strong>Props</strong> — direct parent-child. Explicit and cheapest to reason about.</li>
      <li><strong>Context</strong> — shared config across a subtree. Safe during SSR.</li>
      <li><strong>Classes with <code>$state</code> fields</strong> — app-wide shared state via .svelte.ts modules.</li>
    </ul>
    <p class="note">
      <strong>When NOT to use module-level state:</strong>
      data scoped to a single user/request. During SSR a
...
<style>
  ...
  .nested { margin-left: 1rem; padding: 0.5rem; border-left: 3px solid #6690ff; margin-top: 0.5rem; }
  pre {
    background: #0f172a; color: #e2e8f0; padding: 0.75rem;
    border-radius: 4px; font-size: 0.75rem; overflow-x: auto; white-space: pre;
  }
  ul { padding-left: 1.25rem; }
  li { margin: 0.3rem 0; }
  .note {
    margin-top: 0.75rem; padding: 0.6rem 0.8rem; background: #fff7ed;
    border-left: 3px solid #f59e0b; border-radius: 3px; font-size: 0.85rem; color: #78350f;
  }
  .notify-row { display: flex; gap: 0.5rem; margin: 0.5rem 0; }
  button {
    padding: 0.45rem 0.9rem; border: 1px solid #6690ff; background: #6690ff;
    color: white; border-radius: 4px; cursor: pointer; font-size: 0.85rem;
  }
  .messages {
    padding: 0.5rem 1rem; margin: 0.5rem 0; background: #f9fafb;
    border: 1px solid #e5e7eb; border-radius: 4px; list-style: none; font-size: 0.85rem;
  }
  .messages li { padding: 0.2rem 0; border-bottom: 1px dashed #e5e7eb; }
  .messages li:last-child { border-bottom: none; }
  .messages li.empty { color: #9ca3af; font-style: italic; }
  code { background: #f3f4f6; padding: 0 0.3rem; border-radius: 3px; font-size: 0.85em; }
</style>`,
			language: 'svelte'
		},
		{
...
<style>
  .provider {
    padding: 1rem; border-radius: 8px; transition: all 0.3s;
  }
  .provider[data-theme="light"] { background: #fafafa; color: #222; }
  .provider[data-theme="dark"] { background: #1a1a2e; color: #eee; }
  button { padding: 0.5rem 1rem; cursor: pointer; margin-bottom: 1rem; }
