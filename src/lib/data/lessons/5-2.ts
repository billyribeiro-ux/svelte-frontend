...
<style>
  ...
  // ============================================================
  // SOLUTION — The DOM & Hydration
  // ============================================================
  // Best-practice: typed bind:this, proper null guards, RAF cleanup.

  // === Measured box ===
  let boxElement: HTMLDivElement | null = $state(null);
  let boxWidth: number = $state(0);
  let boxHeight: number = $state(0);
  let boxTop: number = $state(0);
  let boxLeft: number = $state(0);

  let boxColor: string = $state('#ff3e00');
  let boxSize: number = $state(220);
  let boxPadding: number = $state(16);
  ...
  // === Inspect a paragraph's DOM properties ===
  let para: HTMLParagraphElement | null = $state(null);
  ...
  let paraInfo: ParaInfo = $state({ tag: '', childCount: 0, textLength: 0, className: '' });

  function inspectParagraph(): void {
    if (!para) return;
    paraInfo = {
      tag: para.tagName,
      childCount: para.childNodes.length,
  ...
  let hydrationStep: number = $state(0);
  const hydrationSteps: HydrationStep[] = [
    {
      title: 'Step 1 — Server renders HTML',
      body: 'Your Svelte component is rendered to a plain HTML string on the server. No JavaScript yet.',
      code: '<button class="counter">Count: 0</button>'
  ...
  const currentStep = $derived(hydrationSteps[hydrationStep]);
</script>
