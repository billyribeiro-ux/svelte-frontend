	...
	solution: [
		{
			...
			content: `<script lang="ts">
  // ============================================================
  // SOLUTION — Default & Named Actions
  // ============================================================
  // Best-practice: typed action names, discriminated union for
  // action log, clean onsubmit handlers.
...
export const actions: Actions = {
  ...
  <section>
    <h2>2. Named Actions — One Page, Many Mutations</h2>
    <p class="hint">
      When you need more than one mutation on a page, give each action a name
      and target it with <code>action="?/name"</code>.
    </p>
    <pre>{\`// src/routes/auth/+page.server.ts
import type { Actions } from './$types';
import { fail } from '@sveltejs/kit';
...
export const actions: Actions = {
  ...
  <section>
    <h2>3. Multiple Buttons, One Form (formaction)</h2>
    <p class="hint">
      When actions share most fields, use <code>formaction</code>
      on each submit button to pick which action runs.
    </p>
    <div class="demo-block">
      <strong>Try it:</strong> same form, two submit buttons targeting different actions.
