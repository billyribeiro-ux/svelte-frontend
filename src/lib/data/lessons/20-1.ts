...
<style>
  ...
  // ============================================================
  // Best-practice: fully typed data models, clean snippet usage,
  // proper derived state for progress tracking.
  ...
  // ============ Routes ============
  type RenderMode = 'prerender' | 'ssr' | 'csr';
  ...
  // ============ State strategy ============
  type StateKind = 'local' | 'context' | 'module';
...
<main>
  ...
  {:else if activeTab === 'state'}
    <section>
      <h2>State Strategy</h2>
      <p>Default to local. Lift to context when two siblings share it. Reach for a module only when state must outlive component trees.</p>
      <table class="state-table">
        <thead><tr><th>State</th><th>Kind</th><th>Example</th></tr></thead>
        <tbody>
          {#each stateStrategy as s (s.name)}
  ...
  {:else if activeTab === 'render'}
    <section>
      ...
      <div class="mode-grid">
        <div class="mode-card">
          <h3>Prerender</h3>
          <p>Build-time HTML, shipped as static files. Fastest, cheapest, perfectly cacheable.</p>
          <code>export const prerender = true</code>
        </div>
        <div class="mode-card">
          <h3>SSR</h3>
          <p>Server renders on every request. Needed for personalized content and dynamic data.</p>
          <code>export const ssr = true</code>
        </div>
        <div class="mode-card">
          <h3>CSR only</h3>
...
<style>
  ...
  h2 { margin-top: 0; }
  .comp-node {
    display: grid; grid-template-columns: auto auto 1fr; gap: 0.75rem;
    padding: 0.3rem 0.5rem; padding-left: calc(var(--depth) * 1.25rem + 0.5rem);
    align-items: center; font-size: 0.88rem; border-left: 2px solid #e0e0e0;
  }
  .comp-name { font-weight: 700; color: #1e40af; font-family: monospace; }
  .comp-role { color: #666; font-style: italic; font-size: 0.8rem; }
  .comp-shared {
  ...
  .routes, .state-table, .seo { width: 100%; border-collapse: collapse; font-size: 0.86rem; }
  .routes th, .routes td, .state-table th, .state-table td, .seo th, .seo td {
    padding: 0.5rem; text-align: left; border-bottom: 1px solid #e0e0e0;
  }
  .routes th, .state-table th, .seo th { background: #f0f0f0; }
  code { background: #f0f0f0; padding: 0.1rem 0.3rem; border-radius: 3px; font-size: 0.8rem; }
  .render-badge { padding: 0.15rem 0.5rem; border-radius: 999px; font-size: 0.7rem; font-weight: 600; }
  .render-badge.prerender { background: #dcfce7; color: #166534; }
  .render-badge.ssr { background: #dbeafe; color: #1e40af; }
  .render-badge.csr { background: #fef3c7; color: #92400e; }
  .kind { padding: 0.15rem 0.5rem; border-radius: 999px; font-size: 0.72rem; font-weight: 600; }
  ...
  .mode-card { padding: 1rem; background: #f8f9fa; border-radius: 10px; border-left: 4px solid #4a90d9; }
  .mode-card h3 { margin: 0 0 0.35rem; }
  .mode-card p { font-size: 0.85rem; color: #555; margin: 0 0 0.5rem; }
  .mode-card code { font-size: 0.75rem; background: #111; color: #9ae6b4; padding: 0.2rem 0.5rem; border-radius: 4px; }
  .mark-btn {
    margin-top: 1rem; padding: 0.5rem 1.1rem; background: #4a90d9;
  ...
  .summary-card {
    padding: 1rem; background: #f8f9fa; border: 2px solid #e0e0e0; border-radius: 10px;
  }
  .summary-card.done { background: #dcfce7; border-color: #16a34a; }
  .summary-card h3 { margin: 0 0 0.25rem; font-size: 0.95rem; }
  .summary-card p { margin: 0; font-size: 0.8rem; color: #555; }
  .celebrate {
    margin-top: 1.5rem; padding: 1rem; background: #dcfce7;
    border-radius: 10px; text-align: center; font-weight: 700; color: #166534;
  }
</style>`,
			language: 'svelte'
		}
