...
<style>
	...
	solution: [
		{
			...
			content: `<script lang="ts">
  // ============================================================
  // SOLUTION — Checkboxes, Radios, Selects & Details
  // ============================================================
  // Best-practice: all types explicit, descriptive naming,
  // clean derived state for the order summary.
  ...
  // --- Checkbox group (bind:group) ----------------------------
  let toppings: string[] = $state(['Cheese']);
  const availableToppings: string[] = [
    'Cheese', 'Pepperoni', 'Mushrooms', 'Olives',
    'Peppers', 'Onions', 'Pineapple', 'Anchovies'
  ];

  function selectAll(): void { toppings = [...availableToppings]; }
  function clearToppings(): void { toppings = []; }

  // --- Single <select> (bind:value) ---------------------------
  type PizzaSize = 'small' | 'medium' | 'large';
  let size: PizzaSize = $state('medium');
  const sizePrice: Record<PizzaSize, number> = { small: 9, medium: 12, large: 15 };

  // --- Multi <select> (bind:value with array) -----------------
  let languages: string[] = $state(['ts', 'svelte']);
  const allLanguages: { id: string; label: string }[] = [
    { id: 'ts', label: 'TypeScript' },
    { id: 'js', label: 'JavaScript' },
    { id: 'svelte', label: 'Svelte' },
    { id: 'rust', label: 'Rust' },
  ...
  function expandAll(): void { faqOpen = faqOpen.map(() => true); }
  function collapseAll(): void { faqOpen = faqOpen.map(() => false); }
